...
public class PostService {
    ...
    public PostResponse create(PostCreateCommand command) {
        ...
        //테그 입력값 검증필터
        List<String> filteredTagNames = command.tagNames().stream()
        ...
        Place findPlace = placeRepository.findByName(command.placeName())
                .orElseThrow(() -> new AppException(ErrorType.PLACE_NOT_FOUND));

        List<String> filteredCategoryNames = command.categoryNames().stream()
                .filter(StringUtils::hasText)
                .map(String::trim)
                .distinct()
                .toList();

        List<PlaceCategory> findCategories = placeCategoryRepository.findByNameIn(filteredCategoryNames);
        ...
        // 저장 로직
        Post post = Post.of(command.title(),
                command.contents(),
                command.startedAt(),
                command.endedAt(),
                command.studyDate(),
        ...
        List<PostTag> postTags = findTags.stream()
                .map(tag -> PostTag.of(savedPost, tag))
                .toList();
        postTagRepository.saveAll(postTags);

        List<PostCategory> postCategories = findCategories.stream()
                .map(category -> PostCategory.of(savedPost, category))
                .toList();
